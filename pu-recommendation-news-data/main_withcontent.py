...
def min_objective(Xu, Xs, L, last_U, last_V, samples):
	...
	for itera in xrange(nIterations):
		...
		if skipped_elements[i, j] == 1 or (last_U != None and y*np.dot(last_U[i,:], last_V[j, :]) < -1):
		...
		V[j, :] = V[j, :] - learn_rate * (alpha * gradv + (1-alpha) * (2*V[j, :] - 2*L[j,:])) - learn_rate * lambd * 2 * V[j, :]
...
def train(Xu, Xs, L, samples):
	...
	while diff_non_zero > 100 or len(skipped_elements.nonzero()[0]) == 0:
		...
		last_U, last_V = min_objective(Xu, Xs, L, last_U, last_V, samples)
...
def obtain_L(Xs):
	...
	if os.path.isfile('./data/L.pkl'):
		L = cPickle.load(open('./data/L.pkl', 'r'))
		return L
	...
	V = normalize(V, norm='l2', axis=1)
	cPickle.dump(V, open('./data/L.pkl', 'w'))
	return V
